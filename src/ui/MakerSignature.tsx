/**
 * The maker's mark: a quiet author credit at the bottom of the page.
 *
 * It sits at the end of the scrolling derivation rather than inside the sticky
 * bar below it. That bar is a live control surface, not a site footer, and a
 * personal credit does not belong among the cost parameters, nor in the pinned
 * height they are already spending.
 *
 * Everything identifying lives in the two constants below, so updating a link
 * or adding a platform is one line and no markup.
 */
...
interface MakerLink {
  label: string;
  href: string;
  /** 24-unit viewBox, so every glyph shares one grid. */
  path: string;
  /** Brand marks are solid; the globe is drawn in line. */
  stroke?: boolean;
}
...
const LINKS: MakerLink[] = [
  {
    ...
    stroke: true,
    path: 'M12 3a9 9 0 100 18 9 9 0 000-18zM3 12h18M12 3c2.5 2.4 3.9 5.6 4 9-.1 3.4-1.5 6.6-4 9-2.5-2.4-3.9-5.6-4-9 .1-3.4 1.5-6.6 4-9z',
  },
  {
    label: 'GitHub',
...
export function MakerSignature() {
  // Read at render, so the credit does not quietly go stale in January.
  ...
  return (
    <div className="maker">
      <p className="maker-credit">
        ...
        <span className="maker-year">&copy; {year}</span>
      </p>
            ...
            >
              <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
                <path
                  ...
                  strokeWidth={link.stroke ? 1.7 : undefined}
                  strokeLinecap={link.stroke ? 'round' : undefined}
                  strokeLinejoin={link.stroke ? 'round' : undefined}
                />
              </svg>
            </a>
          </li>
